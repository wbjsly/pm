...
@DisplayName("CharterApprovalCallback 测试")
class CharterApprovalCallbackTest {

    @Autowired
    private CharterApprovalCallback callback;

    @Autowired
    private ApprovalCallbackRegistry registry;

    private static final String BIZ_ID = "charter_001";
    private static final String FLOW_CODE = "PM_CHARTER_APPROVAL";
    ...
    class BeanRegistration {
        ...
        @Test
        @DisplayName("Spring 应成功创建 CharterApprovalCallback bean")
        void bean_shouldBeCreated() {
            assertThat(callback).isNotNull();
        }

        @Test
        @DisplayName("getFlowCode 应返回 PM_CHARTER_APPROVAL")
        ...
        void shouldBeRegisteredInRegistry() {
            ...
            assertThat(retrieved).isNotNull().isInstanceOf(CharterApprovalCallback.class);
        }
    }
    ...
    class OnApproved {
        ...
        void onApproved_shouldNotThrow() {
            assertDoesNotThrow(() ->
                    callback.onApproved(BIZ_ID, Map.of("comment", "同意立项")));
        }
    ...
    class OnRejected {
        ...
        void onRejected_shouldNotThrow() {
            assertDoesNotThrow(() ->
                    callback.onRejected(BIZ_ID, "预算不合理"));
        }
