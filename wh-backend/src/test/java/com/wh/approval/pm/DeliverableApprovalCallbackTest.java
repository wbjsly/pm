...
@DisplayName("DeliverableApprovalCallback 测试")
class DeliverableApprovalCallbackTest {

    @Autowired
    private DeliverableApprovalCallback callback;

    @Autowired
    private ApprovalCallbackRegistry registry;

    private static final String BIZ_ID = "deliverable_001";
    private static final String FLOW_CODE = "PM_DELIVERABLE_APPROVAL";
    ...
    class BeanRegistration {
        ...
        @Test
        @DisplayName("Spring 应成功创建 DeliverableApprovalCallback bean")
        void bean_shouldBeCreated() {
            assertThat(callback).isNotNull();
        }

        @Test
        @DisplayName("getFlowCode 应返回 PM_DELIVERABLE_APPROVAL")
        ...
        void shouldBeRegisteredInRegistry() {
            ...
            assertThat(retrieved).isNotNull().isInstanceOf(DeliverableApprovalCallback.class);
        }
    }
    ...
    class OnApproved {
        ...
        void onApproved_shouldNotThrow() {
            assertDoesNotThrow(() ->
                    callback.onApproved(BIZ_ID, Map.of("comment", "交付物验收通过")));
        }
    ...
    class OnRejected {
        ...
        void onRejected_shouldNotThrow() {
            assertDoesNotThrow(() ->
                    callback.onRejected(BIZ_ID, "交付物未达标"));
        }
